                    ...
                    for bert_dropout in "${bert_dropout_list[@]}"; do
                        for ((split_index=0; split_index<5; ++split_index)); do
                            for ((seed=0; seed<5; ++seed)); do
                                python train_cl.py --gpu 0 --method cl --seed $seed --split_index $split_index --ood_method $ood_method --dataset $dataset --l2 $l2 --sim_method $sim_method --bert_dropout $bert_dropout --k $k
                            done  # seed
                        done # split_index
                    done
                done # bert_dropout
